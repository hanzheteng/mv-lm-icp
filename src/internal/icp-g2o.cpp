...
void g2oOptimizer(vector<std::shared_ptr<Frame> >& frames, bool pointToPlane) {
  ...
  for (int src_id = 0; src_id < frames.size(); src_id++) {
    ...
    for (int j = 0; j < srcCloud.neighbours.size(); ++j) {
      ...
      g2o::VertexSE3* vp0 = dynamic_cast<g2o::VertexSE3*>(optimizer.vertices().find(dst_id)->second);  // dstCloud
      g2o::VertexSE3* vp1 = dynamic_cast<g2o::VertexSE3*>(optimizer.vertices().find(src_id)->second);  // srcCloud
