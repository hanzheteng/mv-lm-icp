...
If you do not have access to these libraries for whatever reason, ITERATIVE_SCHUR with SCHUR_JACOBI is an excellent
alternative.
...
This is also the recommended option if you are solving medium sized problems for which DENSE_SCHUR is too slow but
SuiteSparse is not available. Note: If you are solving small to medium sized problems, consider setting
Solver::Options::use_explicit_schur_complement to true, it can result in a substantial performance boost. If you are not
satisfied with SCHUR_JACOBI‘s performance try CLUSTER_JACOBI and CLUSTER_TRIDIAGONAL in that order. They require that
you have SuiteSparse installed. Both of these preconditioners use a clustering algorithm. Use SINGLE_LINKAGE before
CANONICAL_VIEWS.
*/
...
ceres::Solver::Options getOptionsMedium() {
  ...
  // If you are solving small to medium sized problems, consider setting Solver::Options::use_explicit_schur_complement
  // to true, it can result in a substantial performance boost.
  options.use_explicit_schur_complement = true;
  options.max_num_iterations = 50;
...
Sophus::SE3d isoToSophus(const Isometry3d& pose) { return Sophus::SE3d(pose); }
...
Isometry3d pointToPlane_EigenQuaternion(vector<Vector3d>& src, vector<Vector3d>& dst, vector<Vector3d>& nor) {
  ...
  for (int i = 0; i < src.size(); ++i) {
    ...
    // second viewpoint: srcCloud, moves
    ceres::CostFunction* cost_function =
        ICPCostFunctions::PointToPlaneError_EigenQuaternion::Create(dst[i], src[i], nor[i]);
    problem.AddResidualBlock(cost_function, NULL, q.coeffs().data(), t.data());
  }
      ...
      for (auto corr : dstEdge.correspondances) {
        ...
        if (pointToPlane) {
          cost_function = ICPCostFunctions::PointToPlaneErrorGlobal::Create(
              dstCloud.pts[corr.second], srcCloud.pts[corr.first], dstCloud.nor[corr.second]);
        } else {
          cost_function =
              ICPCostFunctions::PointToPointErrorGlobal::Create(dstCloud.pts[corr.second], srcCloud.pts[corr.first]);
        }
        ...
        //                problem.AddResidualBlock(cost_function, srcQ.coeffs().data(),srcT.data(),dstQ.coeffs().data(),
        //                dstT.data());
        problem.AddResidualBlock(cost_function, loss, srcQ.coeffs().data(), srcT.data(), dstQ.coeffs().data(),
                                 dstT.data());

        //                problem.AddResidualBlock(cost_function, NULL,
        //                &cameras[src_id*7],&cameras[src_id*7+4],&cameras[dst_id*7],&cameras[dst_id*7+4]);
      }
    }
  }

#ifdef useLocalParam
  eigen_quaternion::EigenQuaternionParameterization* quaternion_parameterization =
  ...
  for (int src_id = 0; src_id < frames.size(); src_id++) {
    ...
    for (int j = 0; j < srcCloud.neighbours.size(); ++j) {
      ...
      for (auto corr : dstEdge.correspondances) {
        ...
        if (pointToPlane) {
          cost_function = ICPCostFunctions::PointToPlaneErrorGlobal_CeresAngleAxis::Create(
              dstCloud.pts[corr.second], srcCloud.pts[corr.first], dstCloud.nor[corr.second]);
        } else {
          cost_function = ICPCostFunctions::PointToPointErrorGlobal_CeresAngleAxis::Create(dstCloud.pts[corr.second],
  ...
  for (int src_id = 0; src_id < frames.size(); src_id++) {
    ...
    for (int j = 0; j < srcCloud.neighbours.size(); ++j) {
      ...
      for (auto corr : dstEdge.correspondances) {
        ...
        if (pointToPlane) {
          cost_function = ICPCostFunctions::PointToPlaneErrorGlobal_SophusSE3::Create(
              dstCloud.pts[corr.second], srcCloud.pts[corr.first], dstCloud.nor[corr.second]);
        } else {
          cost_function = ICPCostFunctions::PointToPointErrorGlobal_SophusSE3::Create(dstCloud.pts[corr.second],
                                                                                      srcCloud.pts[corr.first]);
        }
...
//        // second viewpoint: srcCloud, moves
//        ceres::CostFunction* cost_function =
//        ICPCostFunctions::PointToPointErrorGlobal_SophusSE3::Create(dst[i],src[i]);
...
//        // second viewpoint: srcCloud, moves
//        ceres::CostFunction* cost_function =
//        ICPCostFunctions::PointToPlaneErrorGlobal_SophusSE3::Create(dst[i],src[i],nor[i]);
//        problem.AddResidualBlock(cost_function, NULL,soph.data(),soph2.data());
//    }
